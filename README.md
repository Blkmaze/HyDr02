# HyDr02 Builder — Android TV / Fire TV IPTV player

Your own "APK builder": fill in a form, get a signed APK a few minutes
later. No Android Studio needed, no reseller panel, no billing, and no
pre-configured server — this is a personal build tool. Every build ships
empty; you (or whoever installs it) add your own Xtream/M3U server on
first launch, the same way you would in TiviMate or IPTV Smarters.
...
1. Open `web/builder.html` directly in a browser (double-click it), or serve it
   ...
   **Settings → Pages → Deploy from a branch → `/web`** on this repo to get it at
   `https://<you>.github.io/HyDr02/builder.html`.
2. Paste in a GitHub [personal access token](https://github.com/settings/tokens/new?description=HyDr02&scopes=repo,workflow)
...
|---|---|
| app_name | `Maze TV` |
| package_id | `com.hydr02.player` (must be unique per brand) |
...
```json
[{"name":"My Server","host":"http://portal.example.com:8080"}, {"name":"Backup Server","host":"http://backup.example.com:8080"}]
```
